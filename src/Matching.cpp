...
int Matching::Blossom(int u, int v)
{
	...
	// cout << "GETFREEBLOSSOMEINDEX" << endl;
	vector<bool> isInPath(2*n, false);

	//Find the tip of the blossom
	int u_ = u; 
	// cout << "u_: " << u_ << endl;
	// cout << "outer[u_]: " << outer[u_] << endl;
	// cout << "outer, below" << endl;
	// for(vector<int>::iterator itr = outer.begin(); itr != outer.end(); itr++)
	// 	cout << *itr << " ";
	// cout << endl;
	// cout << "forest, below" << endl;
	// for(vector<int>::iterator itr = forest.begin(); itr != forest.end(); itr++)
	// 	cout << *itr << " ";
	// cout << endl;
	clock_t start_, end_;
	start_ = clock();
	...
	while(u_ != -1)
	{
		...
		u_ = forest[outer[u_]];
		end_ = clock();
		if ( ((double) (end_ - start_)) / CLOCKS_PER_SEC > 0.0001 ) {
			escape = true;
			// cout << "escape!"<< endl;
			return 0;
		}
	...
	while(not isInPath[v_]) {
		v_ = outer[forest[v_]];
		end_ = clock();
		if ( ((double) (end_ - start_)) / CLOCKS_PER_SEC > 0.0002 ) {
			escape = true;
			// cout << "escape!''"<< endl;
			return 0;
		}
	...
	while(u_ != tip[t])
	{
		...
		circuit.push_front(u_);
		end_ = clock();
		if ( ((double) (end_ - start_)) / CLOCKS_PER_SEC > 0.0005 ) {
			escape = true;
			// cout << "escape!1"<< endl;
			return 0;
		}
	...
	while(v_ != tip[t])
	{
		...
		v_ = outer[forest[v_]];
		end_ = clock();
		if ( ((double) (end_ - start_)) / CLOCKS_PER_SEC > 0.001 ) {
			escape = true;
			// cout << "escape!2"<< endl;
			return 0;
		}
...
pair< list<int>, double> Matching::SolveMinimumCostPerfectMatching(const vector<double> & cost)
{
	...
	while(not perfect)
	{
		...
		if (escape) {
			list<int> wtfshit;
			escape = false;
			return make_pair(wtfshit, -1.0);
		}
		// cout << "Grow" << endl;
		UpdateDualCosts();
		// cout << "UpdateDualCosts" << endl;
